...
from oscillators.van_der_pol_scaled import (
    van_der_pol_jacobian_model_scaled,
...
def main() -> None:
    ...
    van_der_pol_jacobian = van_der_pol_jacobian_model_scaled(
        mi=VanDerPolScaled.MI, d=VanDerPolScaled.D
    )

    # Wrap the oscillator to match solve_ivp signature (t, y) -> dy
    def system(t: float, y: np.ndarray) -> np.ndarray:
        return van_der_pol(y)
    ...
    with ExecutionTimer("Evolution to limit cycle"):
        sol = solve_ivp(
            system,
            (0, evolve_time),
            VanDerPolScaled.initial_conditions,
            method="RK45",
            rtol=1e-7,
            atol=1e-10,
        )
    ...
    plt.plot(sol_plot.y[0], sol_plot.y[1], label="Van Der Pol limit cycle")
    plt.scatter([sol_plot.y[0, 0]], [sol_plot.y[1, 0]], color="red", s=50, zorder=5)
    plt.xlabel("x")
    plt.ylabel("y")
    plt.title("Van Der Pol Limit Cycle")
    plt.grid(True)
    plt.legend()
    plt.show()
    ...
    print(
        f"Floquet exponents: λ_0 = {result.exponents[0]:.6f}, λ_1 = {result.exponents[1]:.6f}"
    )
    ...
    # Save results
    oscillator_params = floquet_result_to_oscillator_params(
        result, VanDerPolScaled.name
    )
    oscillator_params.dump()
    print(f"\nFloquet results saved for oscillator: {VanDerPolScaled.name}")
