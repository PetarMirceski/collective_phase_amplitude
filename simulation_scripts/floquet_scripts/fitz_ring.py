...
def main() -> None:
    ...
    plt.plot(sol_plot.y[1], sol_plot.y[3], label="FitzHugh-Nagumo Ring limit cycle")
    plt.scatter([sol_plot.y[1, 0]], [sol_plot.y[3, 0]], color="red", s=50, zorder=5)
    plt.xlabel("v1")
    plt.ylabel("v2")
    plt.title("FitzHugh-Nagumo Ring Limit Cycle (v1-v2 projection)")
    plt.grid(True)
    plt.legend()
    plt.show()
    ...
    print(f"System dimension: {FitzNagumoRing.n}")
    print(f"Number of oscillators: {FitzNagumoRing.num_of_oscillators}")
    ...
    # Save results
    oscillator_params = floquet_result_to_oscillator_params(result, FitzNagumoRing.name)
    oscillator_params.dump()
    print(f"\nFloquet results saved for oscillator: {FitzNagumoRing.name}")
