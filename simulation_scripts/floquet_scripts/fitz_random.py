...
def main() -> None:
    ...
    plt.xlabel("u1")
    plt.ylabel("v1")
    plt.title("FitzHugh-Nagumo Random Network Limit Cycle (u1-v1 projection)")
    plt.grid(True)
    plt.legend()
    plt.show()
    ...
    print(f"System dimension: {FitzNagumo.n}")
    print(f"Number of oscillators: {FitzNagumo.num_of_oscillators}")
    ...
    # Save results
    oscillator_params = floquet_result_to_oscillator_params(result, FitzNagumo.name)
    oscillator_params.dump()
    print(f"\nFloquet results saved for oscillator: {FitzNagumo.name}")
