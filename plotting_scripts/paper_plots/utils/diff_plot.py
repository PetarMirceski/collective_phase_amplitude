...
def plot_diff(
    ...
    for i, config in tqdm(enumerate(total_states), total=len(total_states)):
        ...
        # )
        axins = inset_axes(axs[i, 0], width="40%", height="40%", loc="center right")
        ...
        axs[i, 1].plot(gamma_amp[:, 0], gamma_amp[:, 1], label="amplitude suppression")

        axs[i, 0].axhline(linewidth=9, linestyle="dashed", color="black", alpha=0.4)
        axs[i, 1].axhline(linewidth=2, linestyle="dashed", color="black")
        axs[i, 1].axvline(linewidth=2, linestyle="dashed", color="black")
        axs[i, 1].set_xticks(
        ...
        axs[i, 0].set_ylabel(r"$\phi$")
        axs[i, 1].set_xlabel(r"$\phi$")
        axs[i, 1].set_ylabel(r"$\Delta + \Gamma(\phi)$")
        # plt.ylim([-0.05, 0.3])
        axs[i, 0].grid()

        axs[i, 0].legend(handles=[l1, l2, l3, l4], prop={"size": 15}, loc="upper right")
        axs[i, 1].legend(prop={"size": 15})

        if top_y_lim != -1:
            axs[i, 0].set_ylim(top=top_y_lim)

        if right_x_lim != -1:
            axs[i, 0].set_xlim(right=right_x_lim)
