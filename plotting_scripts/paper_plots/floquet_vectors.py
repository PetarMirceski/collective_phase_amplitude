...
def main() -> None:
    ...
    plt.savefig(BASE_DIR / "random_net_psf.pdf", bbox_inches="tight")
    plt.show()

    plot_real_psf_isf(fitz_data.left_floquet_vectors[1].real, phase_vector, label="I")
    plt.savefig(BASE_DIR / "random_net_isf.pdf", bbox_inches="tight")
    plt.show()
    ...
    plt.savefig(BASE_DIR / "random_net_limit_cycle.pdf", bbox_inches="tight")
    plt.show()
    # Print to terminal the floquet exponents freqs and everything else
    print("Report for the Random Network")
    ...
    phase_vector = np.arange(0, length_of_data) * step_fitz_phase
    plot_real_psf_isf(fitz_data.left_floquet_vectors[0].real, phase_vector, label="Z")
    plt.savefig(BASE_DIR / "ring_net_psf.pdf", bbox_inches="tight")
    plt.show()
    plot_comp_isf(fitz_data.left_floquet_vectors[1], phase_vector, label="I")
    plt.savefig(BASE_DIR / "ring_net_isf.pdf", bbox_inches="tight")
    plt.show()

    # Limit Cycle plotting of Random network
    time_vector = np.arange(0, length_of_data) * STEP_FITZ
    plot_states(fitz_data.limit_cycle, time_vector)
    plt.savefig(BASE_DIR / "ring_net_limit_cycle.pdf", bbox_inches="tight")
    plt.show()

    print("Report for the Ring Network")
    print(f"The time period T is: {fitz_data.natural_period}")
    print(f"The freq W is: {fitz_data.natural_freq}")
